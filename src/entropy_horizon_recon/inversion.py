...
def infer_logmu_forward(
    *,
    ...
    """
    global _GLOBAL_LOG_PROB, _GLOBAL_LOG_LIKE, _GLOBAL_LOG_PRIOR
    ...
    debug_log_limit = 20
    debug_log_disabled = bool(n_processes is not None and int(n_processes) > 1)
    ...
    def _propose_init_row() -> np.ndarray:
        if valid_rows:
            base = valid_rows[int(rng_init.integers(0, len(valid_rows)))]
            row = base + rng_init.normal(scale=0.03, size=ndim)
            if not mu_is_fixed:
                row[sl_logmu] = base[sl_logmu] + rng_init.normal(scale=0.04, size=n_mu_knots)
            row[idx_log_sig_cc] = base[idx_log_sig_cc] + rng_init.normal(scale=0.12)
            row[idx_log_sig_sn] = base[idx_log_sig_sn] + rng_init.normal(scale=0.12)
            row[idx_log_sig_d2] = base[idx_log_sig_d2] + rng_init.normal(scale=0.25)
            if sl_r_knots is not None:
                row[sl_r_knots] = base[sl_r_knots] + rng_init.normal(scale=0.01, size=n_r)
            return row
        return _init_row(rng_init)

    pending = list(range(n_walkers))
    init_tries = np.zeros((n_walkers,), dtype=int)
    started = set()

    # Parallel validity checks significantly reduce startup time when many cores are available.
    if n_processes and n_processes > 1 and n_walkers >= 8:
        import multiprocessing as mp

        ctx = mp.get_context("fork")
        _GLOBAL_LOG_PROB = log_prob
        try:
            with ctx.Pool(processes=min(int(n_processes), int(n_walkers))) as pool:
                while pending:
                    rows = []
                    for i in pending:
                        if i not in started:
                            ...
                            started.add(i)
                        init_tries[i] += 1
                        if int(init_tries[i]) > int(max_init_tries):
                            raise RuntimeError(
                                "Failed to find a valid initial position after "
                                f"{max_init_tries} attempts; check priors/likelihoods."
                            )
                        rows.append(_propose_init_row())
                    vals = pool.map(_log_prob_wrapper, rows)
                    next_pending = []
                    for i, row, val in zip(pending, rows, vals):
                        if np.isfinite(val):
                            p0[i] = row
                            valid_rows.append(row.copy())
                            _safe_log(f"[init] walker {i+1}/{n_walkers} ok (tries={int(init_tries[i])})")
                        else:
                            next_pending.append(i)
                    pending = next_pending
        except Exception as exc:
            _safe_log(f"[init] parallel validation fallback: {type(exc).__name__}: {exc}")
        finally:
            _GLOBAL_LOG_PROB = None

    # Fallback / completion path for any remaining walkers.
    for i in pending:
        if i not in started:
            _safe_log(f"[init] walker {i+1}/{n_walkers} starting")
            started.add(i)
        while int(init_tries[i]) < int(max_init_tries):
            init_tries[i] += 1
            row = _propose_init_row()
    ...
    try:
        ...
        if sampler_kind_norm == "emcee":
            ...
            def _run_emcee(sampler: "emcee.EnsembleSampler", init_state):
                nonlocal acceptance_fraction_mean, ess_min, ess_by, tau_by, total_steps
                state_local = run_sampler(
                    sampler,
                    state=init_state,
                    n_steps_to_run=int(n_steps),
                ...
                # Convergence enforcement: run up to 2 extensions if ESS is low.
                if total_steps >= 400:
                    ...
                    for _ in range(2):
                        ...
                        if extra <= 0:
                            break
                        state_local = run_sampler(sampler, state=state_local, n_steps_to_run=int(extra), show_progress=False)
            ...
            if n_processes and n_processes > 1:
                ...
                try:
                    with ctx.Pool(processes=n_processes) as pool:
                        ...
                        state = _run_emcee(sampler, initial_state)
            ...
            prior_cache: dict[bytes, tuple[float, str | None]] = {}
            prior_cache_max = 20000

            def _prior_eval_cached(theta_arr: np.ndarray) -> tuple[float, str | None]:
                key = theta_arr.tobytes()
                out = prior_cache.get(key)
                if out is None:
                    out = log_prior(theta_arr)
                    if len(prior_cache) >= prior_cache_max:
                        prior_cache.clear()
                    prior_cache[key] = out
                return out

            def logp_pt(theta: np.ndarray) -> float:
                lp, _reason = _prior_eval_cached(np.asarray(theta, dtype=float))
                return float(lp)
            ...
            def logl_pt(theta: np.ndarray) -> float:
                ...
                lp, lp_reason = _prior_eval_cached(theta_arr)
                if not np.isfinite(lp):
                    reason = f"prior:{lp_reason or 'nonfinite'}"
                    record_invalid(reason)
                    log_invalid(theta_arr, reason)
                    return -np.inf
                t0 = time.perf_counter() if timing_enabled else None
                ll, _params, ll_reason = log_likelihood(
                    theta_arr, timing_acc=timing_acc if timing_enabled else None
                )
                if not np.isfinite(ll):
                    reason = f"like:{ll_reason or 'nonfinite'}"
                    record_invalid(reason)
