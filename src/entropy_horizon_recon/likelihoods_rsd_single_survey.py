...
class RsdFs8CovLogLike:
    ...
    def from_data(cls, *, z: np.ndarray, fs8: np.ndarray, cov: np.ndarray, meta: dict | None = None) -> "RsdFs8CovLogLike":
        ...
        if sign <= 0:
            raise ValueError("RSD covariance has non-positive determinant.")
        order = np.argsort(z)
        z = z[order]
        fs8 = fs8[order]
        cov = cov[np.ix_(order, order)]
        cov_inv = cov_inv[np.ix_(order, order)]
        return cls(z=z, fs8=fs8, cov=cov, cov_inv=cov_inv, logdet=float(logdet), meta=dict(meta or {}))
    ...
    def loglike(self, model: np.ndarray) -> float:
        ...
        ll = -0.5 * (chi2 + self.logdet + self.fs8.size * np.log(2.0 * np.pi))
        return float(ll)
