...
def predict_fsigma8(
    *,
    ...
    """Predict fσ8(z) using the reconstructed background H(z).

    Modes:
      - ``ode``: solve the GR growth ODE and evaluate fσ8 directly.
      - ``gamma``: use the growth-index approximation f(z)=Omega_m(z)^gamma
        with D(z) reconstructed from d ln D / d ln a = f and D(0)=1.
    """
    ...
    mode = str(growth_mode).strip().lower()
    if mode not in {"ode", "gamma"}:
    ...
    gamma = float(growth_gamma)
    if not np.isfinite(gamma) or gamma <= 0.0:
        raise ValueError("growth_gamma must be positive and finite in gamma mode.")

    z_grid = np.asarray(z_grid, dtype=float)
    H_grid = np.asarray(H_grid, dtype=float)
    if z_grid.ndim != 1 or H_grid.ndim != 1 or z_grid.shape != H_grid.shape:
        raise ValueError("z_grid and H_grid must be 1D arrays with matching shape.")
    if z_grid.size < 2 or z_grid[0] != 0.0 or np.any(np.diff(z_grid) <= 0):
        raise ValueError("z_grid must start at 0 and be strictly increasing.")
    if np.any(H_grid <= 0) or not np.all(np.isfinite(H_grid)):
        raise ValueError("H_grid must be positive and finite.")
    if np.any(z_eval < z_grid[0]) or np.any(z_eval > z_grid[-1]):
        raise ValueError("z_eval must lie within the supplied z_grid domain.")

    x_inc = -np.log1p(z_grid)[::-1]
    z_inc = z_grid[::-1]
    H_inc = H_grid[::-1]
    om_a = float(omega_m0) * (1.0 + z_inc) ** 3 * (float(H0) / H_inc) ** 2
    if np.any(~np.isfinite(om_a)) or np.any(om_a <= 0.0):
        raise ValueError("Non-physical Omega_m(a) encountered in gamma growth mode.")
    f_inc = om_a**gamma
    if np.any(~np.isfinite(f_inc)) or np.any(f_inc <= 0.0):
        raise ValueError("Non-finite f(a) encountered in gamma growth mode.")

    c = np.zeros_like(x_inc)
    if x_inc.size > 1:
        dx = np.diff(x_inc)
        c[1:] = np.cumsum(0.5 * (f_inc[:-1] + f_inc[1:]) * dx)
    lnD = c - c[-1]
    D_inc = np.exp(lnD)
    if np.any(~np.isfinite(D_inc)) or np.any(D_inc <= 0.0):
        raise ValueError("Non-physical D(a) in gamma growth mode.")

    x_eval = -np.log1p(z_eval)
    f_eval = np.interp(x_eval, x_inc, f_inc)
    D_eval = np.interp(x_eval, x_inc, D_inc)
    return f_eval * float(sigma8_0) * D_eval
