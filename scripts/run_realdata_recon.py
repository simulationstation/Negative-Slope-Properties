    ...
    *,
    ...
    if max_parallel is not None:
        if int(max_parallel) > 0:
            req = min(req, int(max_parallel))
    elif n_walkers and n_walkers > 0:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Worker processes for μ inference (0=auto; emcee caps at walkers, ptemcee at walkers*ntemps).",
    )
    parser.add_argument("--mu-sampler", type=str, default="emcee", choices=["emcee", "ptemcee"])
    parser.add_argument(
        "--no-ptemcee-fallback",
        action="store_true",
        help="Fail immediately if --mu-sampler ptemcee is unavailable instead of falling back to emcee.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.55,
        help="Constant growth-index gamma used when --growth-mode=gamma.",
    )
    ...
    requested_mu_sampler = str(args.mu_sampler).strip().lower()
    effective_mu_sampler = requested_mu_sampler
    sampler_fallback_reason = None
    if requested_mu_sampler == "ptemcee":
        try:
            if not hasattr(np, "float"):
                np.float = float  # type: ignore[attr-defined]
            if not hasattr(np, "int"):
                np.int = int  # type: ignore[attr-defined]
            import ptemcee  # type: ignore  # noqa: F401
        except Exception as exc:  # pragma: no cover - environment dependent
            if bool(args.no_ptemcee_fallback):
                raise RuntimeError(
                    "Requested --mu-sampler ptemcee, but ptemcee is unavailable in this environment."
                ) from exc
            sampler_fallback_reason = f"ptemcee_unavailable:{type(exc).__name__}:{exc}"
            print(
                "[warn] ptemcee unavailable; falling back to emcee. "
                f"reason={sampler_fallback_reason}",
                flush=True,
            )
            effective_mu_sampler = "emcee"

    if effective_mu_sampler == "ptemcee":
        mu_max_parallel = int(max(1, int(args.mu_walkers))) * int(max(1, int(args.pt_ntemps)))
    else:
        mu_max_parallel = int(max(1, int(args.mu_walkers)))
    args.mu_procs = _resolve_procs(
        args.mu_procs,
        n_walkers=int(args.mu_walkers),
        cpu_cores=cpu_cores,
        max_parallel=mu_max_parallel,
    )
    ...
    infer_kwargs = dict(
        ...
        pk_like=pk_like,
        constants=constants,
        sampler_kind=str(effective_mu_sampler),
    ...
    summary = {
        ...
        "settings": {
            ...
            "mu_sampler_requested": str(requested_mu_sampler),
            "mu_sampler": str(effective_mu_sampler),
            "mu_sampler_fallback_reason": sampler_fallback_reason,
            "pt_ntemps": int(args.pt_ntemps),
            "pt_tmax": float(args.pt_tmax) if args.pt_tmax is not None else None,
