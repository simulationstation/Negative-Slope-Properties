...
To verify that sign-level statements are identifiable under finite Monte Carlo noise, we retain the power-map calibration used in the production pipeline. To quantify the pipeline's ability to distinguish a true slope signal from noise, we compute true-positive rates (TPR) for injected slope anomalies of strength $\nu$. With fixed threshold $\tau=0.06459924567842923$ (false-alarm control), the baseline run gives
\begin{align}
\mathrm{TPR}_{\nu=0.2}&=0.3432,\quad
\mathrm{TPR}_{\nu=0.5}=0.3805,\quad
\mathrm{TPR}_{\nu=0.8}=0.4892,\\
